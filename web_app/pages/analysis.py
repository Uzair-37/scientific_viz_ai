...
def _process_data_for_analysis(data, domain):
    ...
    if domain == "finance":
        ...
        if isinstance(data, pd.DataFrame):
            ...
            # Handle multi-column DataFrames by taking first numeric column
            if isinstance(returns, pd.DataFrame) and len(returns.columns) > 1:
                ...
                if len(numeric_cols) > 0:
                    ...
                    returns_series = returns[first_col]
                    features = pd.DataFrame({first_col: returns_series})
                else:
            ...
            # Add rolling statistics to the features DataFrame
            for window in [5, 20]:
                vol_series = returns_series.rolling(window=window).std()
                features[f'volatility_{window}d'] = vol_series
